...
class TestSharediHLDQN(unittest.TestCase):
    ...
    def test_compute_target(self) -> None:
        ...
        next_q_logits = self.q.network.apply_fn(self.q.params, sample.next_state)[idx_network]
    ...
    def test_loss(self) -> None:
        ...
        # shape (batch_size, n_networks, n_actions, n_bins)
        predictions = self.q.network.apply_fn(self.q.params, samples.state)[:, 1:]
        # shape (batch_size, n_networks, n_bins)
    ...
    def test_best_action(self):
        ...
        idx_network = jax.random.randint(self.key, (), 0, self.n_networks)
        q_logits = self.q.network.apply_fn(self.q.params, state)[idx_network]
    ...
    def test_shift_params(self):
        ...
        self.q.params["params"][f"Dense_{self.q.last_idx_mlp}"]["bias"] = (
            jnp.arange(self.n_networks * self.n_actions * self.n_bins) / 100
        )
