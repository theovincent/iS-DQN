...
class TestiSHLDQN(unittest.TestCase):
    ...
    def test_compute_target(self) -> None:
        ...
        self.assertEqual(next_q_logits.shape, (2 * self.n_bellman_iterations, self.n_actions, self.n_bins))
    ...
    def test_loss(self) -> None:
        ...
        predictions = self.q.network.apply_fn(self.q.params, samples.state)[:, self.n_bellman_iterations :]
        ...
        # shape (batch_size, n_bellman_iterations, n_actions, n_bins)
        next_q_logits = self.q.network.apply_fn(self.q.params, samples.next_state)[:, : self.n_bellman_iterations]
    ...
    def test_best_action(self):
        ...
        idx_network = jax.random.randint(self.key, (), 0, self.n_bellman_iterations)
        q_logits = self.q.network.apply_fn(self.q.params, state)[self.n_bellman_iterations + idx_network]
    ...
    def test_shift_params(self):
        ...
        self.q.params["params"][f"Dense_{self.q.last_idx_mlp}"]["bias"] = (
            jnp.arange(2 * self.n_bellman_iterations * self.n_actions * self.n_bins) / 100
        )
