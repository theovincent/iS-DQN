...
from slimdqn.networks.shareddghldqn import SharedDGHLDQN
from tests.utils import Generator


class TestSharedDGHLDQN(unittest.TestCase):
    ...
    def test_compute_target(self) -> None:
        ...
        next_q_logits = self.q.network.apply_fn(self.q.params, sample.next_state)[0]

        computed_target = self.q.compute_target(sample, next_q_logits)

        target = sample.reward + (1 - sample.is_terminal) * self.q.gamma * jnp.max(
            jax.nn.softmax(next_q_logits) @ self.q.bin_centers
        )
    ...
    def test_loss(self) -> None:
        ...
        # shape (batch_size, n_actions, n_bins)
        ...
        # shape (batch_size, n_actions, n_bins)
        next_q_logits = self.q.network.apply_fn(self.q.params, samples.next_state)[:, 0]
        targets = self.q.compute_target(samples, next_q_logits)
        projected_targets = jax.vmap(self.q.project_target_on_support)(targets)[0]

        loss = optax.softmax_cross_entropy(q_logits, projected_targets).mean()
    ...
    def test_best_action(self):
        ...
        q_logits = self.q.network.apply_fn(self.q.params, state)[1]
    ...
    def test_shift_params(self):
        ...
        self.q.params["params"][f"Dense_{self.q.last_idx_mlp}"]["bias"] = (
            jnp.arange(2 * self.n_actions * self.n_bins) / 100
        )
