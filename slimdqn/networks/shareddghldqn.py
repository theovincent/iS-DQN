...
class SharedDGHLDQN:
    def __init__(
        ...
        self.network = DQNNet(features, architecture_type, 2 * n_actions * self.n_bins, layer_norm)
        self.network.apply_fn = lambda params, state: jnp.squeeze(
            self.network.apply(params, state).reshape((-1, 2, n_actions, self.n_bins))
        )
    ...
    def update_target_params(self, step: int):
        if step % self.target_update_frequency == 0:
            ...
            logs[f"networks/0_unsupported_prob"] = self.cumulated_unsupported_prob / (
    ...
    def loss_on_batch(self, params: FrozenDict, samples):
        ...
        # shape (2 * batch_size, 2, n_actions, n_bins)
        ...
        # shape (batch_size)
        targets = self.compute_target(samples, all_q_logits[batch_size:, 0])
        # shape (batch_size, n_bins) | (batch_size)
        projected_targets, unsupported_probs = jax.vmap(self.project_target_on_support)(targets)

        # shape (batch_size)
        cross_entropies = optax.softmax_cross_entropy(q_logits, projected_targets)
        entropies = -jnp.sum(projected_targets * jnp.log(projected_targets), axis=-1)

        return cross_entropies.mean(), unsupported_probs.mean(), entropies.mean()
    ...
    def best_action(self, params: FrozenDict, state: jnp.ndarray, **kwargs):
        q_logits = self.network.apply_fn(params, state)[1]
