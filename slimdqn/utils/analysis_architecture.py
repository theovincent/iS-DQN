...
from slimdqn.networks.architectures.dqn import Stack
    ...
    def __call__(self, x, use_running_average=False):
        ...
        for _ in range(2):
            ...
            if self.layer_norm:
                x = nn.LayerNorm()(x)
            x = nn.relu(x)
            score_neurons.append(x.sum(axis=0))
...
class AnalysisNet(nn.Module):
    ...
    def __call__(self, x, use_running_average=False):
        ...
        if self.architecture_type == "cnn":
            ...
            if self.batch_norm:
                x = nn.BatchNorm(use_running_average, axis=(1, 2))(x)
            x, score_neurons_stack = Stack(self.features[0], self.layer_norm, self.batch_norm)(x, use_running_average)
            score_neurons.extend(score_neurons_stack)
            x, score_neurons_stack = Stack(self.features[1], self.layer_norm, self.batch_norm)(x, use_running_average)
            score_neurons.extend(score_neurons_stack)
            x, score_neurons_stack = Stack(self.features[2], self.layer_norm, self.batch_norm)(x, use_running_average)
            score_neurons.extend(score_neurons_stack)
            if self.layer_norm:
                x = nn.LayerNorm()(x)
            x = nn.relu(x).reshape((x.shape[0], -1))
