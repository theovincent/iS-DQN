...
SHARED_ARGS="$SHARED_ARGS --repeat_action_probability $REPEAT_ACTION_PROBABILITY --features $FEATURE $FEATURE \
    --target_update_frequency $TARGET_UPDATE_FREQ \
    --experiment_name rap${REPEAT_ACTION_PROBABILITY}_f${FEATURE}_target_freq${TARGET_UPDATE_FREQ}_n_net${N_NETWORS}_sync_freq${TARGET_SYNC_FREQ}"

# ----- L2 Loss -----
# launch_job/lunar_lander/cluster_dqn.sh --first_seed 1 --last_seed 20 --n_parallel_seeds 1 $SHARED_ARGS
# sleep 20

# iDQN_ARGS="--n_networks $N_NETWORS --target_sync_frequency $TARGET_SYNC_FREQ"
# launch_job/lunar_lander/cluster_idqn.sh --first_seed 1 --last_seed 20 --n_parallel_seeds 1 $SHARED_ARGS $iDQN_ARGS
# sleep 20

# aGIDQN_ARGS="--n_networks $N_NETWORS"
# launch_job/lunar_lander/cluster_agidqn.sh --first_seed 1 --last_seed 20 --n_parallel_seeds 1 $SHARED_ARGS $aGIDQN_ARGS
# sleep 20
